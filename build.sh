...
# Force-replace with the correct Python 3.14 runtime (no-op if not installed).
PLR_SRC="$(python3 -c "import _polars_runtime_32._polars_runtime as m; print(m.__file__)" 2>/dev/null || true)"
PLR_DST="dist/sphere-cli/_internal/_polars_runtime_32/_polars_runtime.abi3.so"
if [[ -n "$PLR_SRC" && -f "$PLR_SRC" && -f "$PLR_DST" ]]; then
